...
Commit `afca22d` ("Polish Football Night home and language switcher") is the current local and `origin/main` baseline. It gives Standard and Football Night distinct fixed-mode homes while retaining one backend, removes the pre-paint legacy selector flash, and keeps Football's language switcher above the localized home. `quizandchill.fun` is the Standard entry and `https://quizandchill.fun/football` is the Football Night entry; the intended `football.quizandchill.fun` entry is code-ready but blocked by the current Railway Trial custom-domain limit. The 317-question difficulty/bank expansion, women's-football exclusion, era-aware distractors, and tighter Football lobby/Start spacing described above are local and not yet published. Final local QA on 2026-08-01 passed `football:check`, `validate:football` (317; 57/110/127/23), all 52 tests, build, and `git diff --check`; five sampled Spanish games opened with recognized trophies/history rather than elementary rules, and a 390×844 browser check placed Start Game in the initial viewport without horizontal overflow. The release retains deterministic prompt IDs, ID-first repeat protection, the editorial validator, continuous music on home return, the compact reveal strip, modern dry effects, synchronized countdown/Start triggers, asset cache-version bumps, multiplayer timer-flicker protection, and targeted same-question state synchronization. The removed Sound check was QA-only and must not be restored to production unless explicitly requested as a development-only tool.
