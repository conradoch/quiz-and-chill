...
Commit `9878f4e` ("Make room creation resilient") is the current published `origin/main` baseline. It gives Standard and Football Night distinct fixed-mode homes while retaining one backend, publishes the 317-question Football bank/harder progression, keeps mobile live/reveal geometry identical, and makes Create/Join recover from transient Socket.IO failure without a page reload while duplicate requests are idempotent server-side. `quizandchill.fun` is the Standard entry and `https://quizandchill.fun/football` is the Football Night entry; the intended `football.quizandchill.fun` entry is code-ready but blocked by the current Railway Trial custom-domain limit. A local, not-yet-published follow-up defaults Football Night to Spanish unless the browser has an explicit saved language and gives standings an opaque product-colored surface so the moon cannot wash out score text. The release retains deterministic prompt IDs, ID-first repeat protection, the editorial validator, continuous music on home return, modern dry effects, synchronized countdown/Start triggers, asset cache-version bumps, multiplayer timer-flicker protection, and targeted same-question state synchronization. The removed Sound check was QA-only and must not be restored to production unless explicitly requested as a development-only tool.
